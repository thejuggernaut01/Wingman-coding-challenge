  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'grid min-w-[9.5rem] items-start gap-1.5 rounded-[8px] border border-transparent bg-white px-2.5 py-2 text-xs shadow-xl',
          className,
        )}
      >
        ...
        {payload.map((item) => {
          ...
          return (
            <div
              key={item.value}
              className={cn(
                'flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground hover:bg-red-0',
              )}
            >
              {itemConfig?.icon && !hideIcon ? (
                ...
                <div
                  className="h-[6px] w-5 shrink-0 rounded-[2px]"
                  style={{
